...
public class ReflectionDisplay {
    ...
    public static void printClassInfo(Class<?> clazz) {
        System.out.println(StringUtils.padCenter("Class Info", 70, '-'));
        System.out.println(ReflectionDisplay.getClassInfo(clazz));
        System.out.println(" ");

        System.out.println(StringUtils.padCenter("Constructor Info", 70, '-'));
        ReflectionDisplay.getConstructorInfo(clazz).forEach(System.out::println);
        System.out.println(" ");

        System.out.println(StringUtils.padCenter("Field Info", 70, '-'));
        ReflectionDisplay.getFieldInfos(clazz).forEach(System.out::println);
        System.out.println(" ");

        System.out.println(StringUtils.padCenter("Method Info", 70, '-'));
        ReflectionDisplay.getMethodInfos(clazz).forEach(System.out::println);
        System.out.println(" ");
        System.out.println(StringUtils.padCenter("", 70, '-'));
    }
    ...
    public static String getClassInfo(Class<?> clazz, String format) {
        ...
        templater.put("interfaces", Arrays.stream(clazz.getInterfaces()).map(Class::getName).collect(Collectors.joining(", ")));
    ...
    public static String getClassInfo(Class<?> clazz) {
        return getClassInfo(clazz, "Class: {name}\nPackage: {package}\nSuperclass: {superclass}\nInterfaces: {interfaces}");
    }
    ...
    /**
     * Returns a list of fields in the class with the format "{name}({type} {modifiers} {fulltype})"
     * <p>
     * Valid placeholders:
     * <ul>
     *     <li>{name} - The name of the field</li>
    ...
    public static List<String> getMethodInfos(Class<?> clazz, String format) {
        ...
        for (var method : clazz.getDeclaredMethods()) {
            ...
            templater.put("exceptions", Arrays.stream(method.getExceptionTypes()).map(Class::getName).collect(Collectors.joining(", ")));
    ...
    /**
     * Returns a list of methods in the class with the format "{modifiers} {returntype} {name}({parameters}) throws {exceptions}"
     * <p>
     * Valid placeholders:
     * <ul>
     *     <li>{name} - The name of the method</li>
    ...
    public static List<String> getMethodInfos(Class<?> clazz) {
        return getMethodInfos(clazz, "{modifiers} {returntype} {name}({parameters}) throws {exceptions}");
    }
    ...
    public static List<String> getConstructorInfo(Class<?> clazz) {
        return getConstructorInfo(clazz, "{modifiers} {name}({parameters}) \nAnnotations: {annotations}");
    }

    private static String getModifiers(int modifiers) {
        return Modifier.getModifiers(modifiers).stream().map(Modifier::getName).collect(Collectors.joining(" "));
    }
